  ...
  try {
    if (req.method === "GET") {
      ...
      if (action === "get_anexos") {
        ...
        const { rows } = await pool.query(`SELECT anexos FROM lancamentos WHERE id = $1`, [id]);
    ...
    if (req.method === "POST") {
      ...
      if (action === "add_anexo") {
        ...
        const { rows } = await pool.query(
          `UPDATE lancamentos SET anexos = COALESCE(anexos, '[]'::jsonb) || $2::jsonb WHERE id = $1 RETURNING anexos`,
