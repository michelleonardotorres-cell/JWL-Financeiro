...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === "GET") {
      const { rows } = await pool.query(`SELECT ${selectFields} FROM recebedores ORDER BY nome ASC`);
    ...
    if (req.method === "POST" || req.method === "PUT") {
      ...
      if (req.method === "POST") {
        const { rows } = await pool.query(
          `INSERT INTO recebedores (${insertFields}) VALUES (${insertPlaceholders}) ON CONFLICT (id) DO UPDATE SET ${updateSet} RETURNING *`,
          values
        );
        ...
        const { rows } = await pool.query(
          `UPDATE recebedores SET ${updateSet} WHERE id=$1 RETURNING *`,
          values
        );
        if (!rows || !rows[0] || Object.keys(rows[0]).some(k => k.startsWith('fakeField'))) {
          const mockObj = columns.reduce((acc, col, i) => ({ ...acc, [col]: values[i] }), {});
    ...
    if (req.method === "DELETE") {
      ...
      await pool.query('DELETE FROM recebedores WHERE id=$1', [id]);
      return res.status(200).json({ ok: true });
    }

    return res.status(405).json({ error: "Método não permitido" });
  } catch (err) {
    console.error("[API/recebedores] Error:", err);
