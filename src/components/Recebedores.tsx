...
export default function Recebedores() {
  ...
  const defaultFormData: Omit<Recebedor, "id"> = {
    nome: "",
    nomeFantasia: "",
  ...
  const [formData, setFormData] = useState<Omit<Recebedor, "id">>(defaultFormData);

  const term = normalizeString(searchTerm);

  const filtered = recebedores.filter((f) => {
  ...
  const uniqueSegmentos = Array.from(new Set(recebedores.map(f => f.segmento).filter(Boolean)));
  const uniqueCidades = Array.from(new Set(recebedores.map(f => f.cidade).filter(Boolean)));

  const openModal = (f?: Recebedor, viewMode: boolean = false) => {
    if (f) {
      setEditingId(f.id);
      setFormData({
        ...defaultFormData,
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await updateRecebedor({ ...formData, id: editingId });
      } else {
        await addRecebedor(formData);
      }
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Deseja realmente excluir este Recebedor?")) {
      try {
        await deleteRecebedor(id);
      } catch (err) {
        alert("Erro ao excluir Recebedor.");
      }
    }
  };
  ...
  const handleDeleteSelected = async () => {
    ...
    if (window.confirm(`Deseja excluir ${selectedIds.size} Recebedores selecionados?`)) {
  ...
  const handleExport = (type: 'all' | 'selected' | 'displayed') => {
    let sourceData = [];
    if (type === 'all') sourceData = recebedores;
    else if (type === 'selected') sourceData = recebedores.filter(f => selectedIds.has(f.id));
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Recebedores");
    XLSX.writeFile(wb, "Recebedores.xlsx");
    setShowOptions(false);
  };
        ...
        try {
          ...
          if (result.errors.length > 0) {
            ...
            if (result.imported > 0) {
              alert(`${result.imported} recebedores importados com sucesso! ${result.errors.length} apresentaram inconsistências.`);
            }
          } else {
            alert(`${result.imported} recebedores importados com sucesso!`);
          }
        } catch (error: any) {
  ...
  const handleRetryImport = async (correctedData: any[]) => {
    ...
    if (result.errors.length > 0) {
      setImportErrors(result.errors);
      alert(`${result.imported} recebedores corrigidos foram importados. ${result.errors.length} ainda apresentam erros.`);
    } else {
      setImportErrors([]);
      alert(`${result.imported} recebedores corrigidos importados com sucesso!`);
    }
  };
    ...
    <div className="flex-1 p-6 overflow-auto bg-zinc-50 relative text-sm">
      <div className="max-w-full mx-auto space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="w-5 h-5 bg-zinc-200 rounded text-zinc-500 flex items-center justify-center cursor-help" title="Gerencie seus Recebedores">
            <Info size={14} />
          </div>
        </div>
              ...
              {showOptions && (
                <div className="absolute right-0 mt-1 w-64 bg-white border border-zinc-200 rounded shadow-lg z-10 py-1">
                  ...
                  <button onClick={() => handleExport('all')} className="w-full text-left px-4 py-2 hover:bg-zinc-100 flex items-center gap-2 text-zinc-700">
                    <Download size={14} /> Todos os Recebedores
                  </button>
          ...
          <div className="bg-[#f3f4f6] shadow-xl w-full max-w-6xl overflow-hidden flex flex-col h-[90vh] rounded border border-zinc-300">
            <div className="p-4 border-b border-zinc-200 flex justify-between items-center bg-[#f8f9fa]">
              <h2 className="text-xl font-bold text-[#5e6e82] flex items-center gap-2 uppercase tracking-wide">
                {isViewing ? "Ficha de Cadastro" : (editingId ? "Editar Recebedor" : "Novo Recebedor")}
            ...
            <div className="flex-1 overflow-y-auto p-4">
              ...
              <div className="flex-1 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block mb-1 font-medium">Qualificação do Recebedor:</label>
